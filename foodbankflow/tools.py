...
Design note: the seed files (inventory, intake queue) are immutable. Every report
works on 'inventory + logged donations', so tools are idempotent and the demo is
repeatable. log_donations reports what logging the queue adds; a real deployment
would persist to a warehouse table here."""
...
def _current_inventory() -> list[dict]:
    ...
    return core.apply_donations(core.load_inventory(), core.load_intake())
...
def log_donations() -> dict:
    ...
    return {
        "logged": [d["donor"] for d in core.load_intake()],
...
def shortage_report() -> dict:
    ...
    return core.shortages(_current_inventory(), core.load_families())


@tool
def family_pick_list(family_id: str) -> dict:
    """A fair, constraint-aware pick list for one family (expiring items first)."""
    fam = next((f for f in core.load_families()["families"] if f["id"] == family_id), None)
    if not fam:
